...
namespace SlimShader
{
	public class BytecodeContainer
	{
		...
		public BytecodeContainer(byte[] rawBytes, ProgramType programType)
		{
			_rawBytes = rawBytes;
			...
			for (uint i = 0; i < Header.ChunkCount; i++)
			{
                ...
                var chunk = BytecodeChunk.ParseChunk(chunkReader, this, programType);
                if (chunk != null)
    				Chunks.Add(chunk);
			}
		}

		public static BytecodeContainer Parse(byte[] bytes, ProgramType programType)
		{
			return new BytecodeContainer(bytes, programType);
		}
